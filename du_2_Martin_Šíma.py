...
def gap_detect(current_date, Date, gap_week : int, gap_pre : int, week_days : int) -> tuple:    # Detekce a výpis chybějících dní
    tmp = gap_week
    if current_date == None:
        ...
        if current_date + datetime.timedelta(days=1) != Date:
            for i in range(1, int((Date - current_date).days)):
                ...
                gap_week += 1
        current_date = Date
    if week_days + tmp >= 7:       # mezery jsou detekovány před možným výpisem týdne, pokud je jsou splněny podmínky pro výpis týdne, program funkce si uloží počet mezer do příštího týdne 
        gap_pre = gap_week - tmp
        gap_week = tmp
    else:
        gap_week += gap_pre
        gap_pre = 0
    
    return gap_week, gap_pre
...
def analyze_by_day(reader, r, writer_week, writer_year) -> None:    # Načtení jednoho záznamu, kontrola validity
    ...
    gap_pre = 0             # Počet mezer spadajících do nového týdne, když předchozí ještě nebyl vytištěn
    ...
    for line in reader:                  # iterace přes všechny řádky souboru
        ...
        if first_row:           # inicializace maximálního a minimálního průtoku
            ...
            first_row = False
        gap_week, gap_pre = gap_detect(current_date, Date, gap_week, gap_pre, week_days)
        ...
        if week_days + gap_week + gap_pre == 0:
            desc_week = (row[0], row[1], row[2], row[3], row[4])
        elif week_days + gap_week + gap_pre >= 7:
